function [mx_est,me_est] = DFLearning(mx_init,me_init,mx_org,IsStructure)
        ...
        if(state < 3)
            ...
            [oDistriList,oGradList,oDeconv,oEnvidence] = ObjEnvidence.calcEnvidence(inDistriList,directionList,inDeconv,state,step_len,compNum);
             ...
             if(IsParaCompare)
                ...
                %
                CataParaList = [CataParaList;tmp.CataNum tmp.pi_x,tmp.CataDegree];
            end
...
save(ParaFileName,'ParaList','OptParaList','StateList','CataParaList');
% figure;
% subplot(2,3,1);
% imagesc(out_mx)
% axis square
