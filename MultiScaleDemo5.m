...
%         dims = dims + (1-mod(dims,2)); %% make odd size
    ...
    for s = 1:NumScale
        if strcmp(RESIZE_OBJ,'grad')
            if(s == 1)
                [tmpGrad,tmpPSF] = InitialPara(obsImScale_grad{s},obsImScale_grad{s},PSFScale{s},0);
                ...
                %[tmpGrad,tmpPSF] = InitialPara(obsImScale_grad{s},newImScale_grad,new_kernel);
               
                [OutImScale_grad{s},OutPSFScale{s}] = SingleScaleProcess6(obsImScale_grad{s},...
                    newImScale_grad,new_kernel,1);
...
finalPSF = getNormImg(OutPSF);
imwrite(uint8(finalPSF*200),['fdir\' OutDirName '\' SingleName '.jpg'],'jpg');